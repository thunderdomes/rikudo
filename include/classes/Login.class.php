...
class Login {
	...
	public function IsLoggedInAsAdmin()
	{
		if (!$this->IsLoggedIn()) return false;
		$account_type = Session::Get('session_account_type');
		if($account_type == 'owner' || $account_type == 'mainadmin' || $account_type == 'admin' || $account_type == 'hotelowner') return true;
		return false;
	}
	...
	{	
		...
		if($this->IsLoggedInAsAdmin()){
			...
			draw_block_top(_ACCOUNTS_MANAGEMENT, $menu_index++);
				...
				if($this->IsLoggedInAs('owner','mainadmin')) echo '<li>'.$this->PrepareLink('accounts_statistics', _STATISTICS).'</li>';
				...
				if(Modules::IsModuleInstalled('customers') && $this->IsLoggedInAs('owner','mainadmin','hotelowner')){
			...
			draw_block_top(_HOTELS_MANAGEMENT, $menu_index++); 
				echo '<div class="menu_category">';
					...
					if($this->IsLoggedInAs('owner','mainadmin') || ($this->IsLoggedInAs('hotelowner') && $this->HasPrivileges('edit_hotel_info'))) echo '<li>'.$this->PrepareLink('mod_booking_meal_plans', _MEAL_PLANS).'</li>';
					if($this->IsLoggedInAs('owner','mainadmin')) echo '<li>'.$this->PrepareLink('mod_rooms_settings', _ROOMS_SETTINGS).'</li>';
					if($this->IsLoggedInAs('owner','mainadmin') || ($this->IsLoggedInAs('hotelowner') && $this->HasPrivileges('edit_hotel_rooms'))) echo '<li>'.$this->PrepareLink('mod_rooms_management', _ROOMS_MANAGEMENT).'</li>';
					echo '</ul>';
				echo '</div>';						
			draw_block_bottom();				
