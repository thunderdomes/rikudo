	...
	{		
		if(!empty($room_id)){
			...
			if(isset($this->arrReservation[$room_id])){
				...
				$this->arrReservation[$room_id]['guests_fee'] = number_format($guest_fee * $nights * $rooms, 2, '.', '');
			}else{
				// just add new room
				$this->arrReservation[$room_id] = array(
					...
					'guests'    => $guests,
					'guests_fee' => number_format($guest_fee * $nights * $rooms, 2, '.', '')
				);
			}
	...
	public function ShowReservationInfo()
	{
		...
		if(count($this->arrReservation) > 0)
		{
				...
				if($result[1] > 0){			
					...
					echo '<tr>
					    ...
					    <tr><td colspan="11" align="'.$class_left.'">
								...
								<table width="100%">
								<tr>
									<td width="90px"><img src="images/rooms_icons/'.$room_icon_thumb.'" alt="" width="80px" height="65px" /></td>
									<td>
			...
			echo '<tr>
					...
					<td class="td '.$class_right.'" align="'.$class_right.'"><b>'.Currencies::PriceFormat($order_price, '', '', $this->currencyFormat).'</b></td>
				</tr>';				
			...
			if($this->vatIncludedInPrice == 'no'){
				echo '<tr> 
						...
						<td class="td '.$class_right.'" align="'.$class_right.'"><b>'.Currencies::PriceFormat($vat_cost, '', '', $this->currencyFormat).'</b></td>
					</tr>';
			}
			echo '<tr><td colspan="11" nowrap height="5px"></td></tr>
				<tr class="footer">
					...
					<td class="td '.$class_right.'" align="'.$class_right.'"><b>'.Currencies::PriceFormat($cart_total, '', '', $this->currencyFormat).'</b></td>
				</tr>
