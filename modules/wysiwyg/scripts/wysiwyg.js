 ...
 *
 * Changes by ApPHP:
 * #001 - 06.03.2011 - commented alert(e);, bacause gives error om page with many text!
 * #002 - 18.04.2011 - jQuery(); replaced with jQuery();
 * #003 - 06.10.2011 - added new icon - InsertDateTime
 * 
 ********************************************************************/
var WYSIWYG = {
	...
	"insertdatetime": ['InsertDateTime', 	   'Insert current date',   'insert_date.gif', 		  'insert_date_on.gif']
	},
	...
	getEditorTable: function(n) {
		return wjQuery("wysiwyg_table_" + n);
	},
	...
	_generate: function(n, settings) {
		...
		if(wjQuery("textMode" + n)) {
			wjQuery("textMode" + n).style.display = 'none'; 
		}
	...
	updateStatusBar: function(n) {
		...
		// update the status bar 	
		var statusbar = wjQuery("wysiwyg_statusbar_" + n);
		if(statusbar){ 
	...
	execCommand: function(n, cmd, value) {
		...
		switch(cmd) {
			...
			case "InsertDateTime": 
				...
				this.insertHTML(year+'-'+month+'-'+day+' '+hour+':'+minite+':'+second, n);
			break;
			...
			case "Save":
			    ...
			    if(form == null) {
			    	alert("Cannot submit the content, because no form element found.");
			    	return;
			    }
			    form.submit();
			break;
	...
	writeDropDown: function(n, id) {
		...
		output += '<td onMouseOver="wjQuery(\'img_' + dropdown.id + '_' + n + '\').src=\'' + imageOn + '\';" onMouseOut="wjQuery(\'img_' + dropdown.id + '_' + n + '\').src=\'' + image + '\';">';
	...
	closeDropDowns: function(n, exid) {
		...
		for(var id in dropdowns) {
			...
			if(dropdown.id != exid) {
				...
				if(wjQuery(divId)) wjQuery(divId).style.display = 'none';
			}
		}			
	},
	...
	openDropDown: function(n, id) {
		...
		if(wjQuery(divId).style.display == "none") {	
			wjQuery(divId).style.display = "block"; 
		}
		else {
			wjQuery(divId).style.display = "none"; 
		}
		wjQuery(divId).style.position = "absolute";
	...
	viewSource: function(n) {
		...
		if(wjQuery('HTMLMode' + n)) {
		    wjQuery('HTMLMode' + n).style.display = 'none'; 
		}
	    if(wjQuery('textMode' + n)) {
		    wjQuery('textMode' + n).style.display = 'block';
		}
	...
	viewText: function(n) { 
		...
		if(wjQuery('HTMLMode' + n)) {
			wjQuery('HTMLMode' + n).style.display = 'block';
		}
	...
	hideToolbars: function(n) {
		for(var i=0;i<this.config[n].Toolbar.length;i++) {
			var toolbar = wjQuery("toolbar" + i + "_" + n);
