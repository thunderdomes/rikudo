...
class Modules extends MicroGrid {
	...
	function __construct($id = "")
	{
		...
		if($this->id != ""){
			$sql = "SELECT
						id, name, icon_file, module_tables, dependent_modules, is_installed, is_system, priority_order, settings_access_by
					FROM ".TABLE_MODULES."
		...
		$this->EDIT_MODE_SQL = "SELECT
								...
								IF(".$this->tableName.".is_system, '"._YES."', '"._NO."') as is_system,
		...
		$this->arrEditModeFields = array(		
			...
			"is_system"         => array("title"=>_SYSTEM_MODULE, "type"=>"label"),
	...
	{		
		$sql = "SELECT id, name, icon_file, module_tables, dependent_modules, is_installed, is_system, priority_order, settings_access_by
				FROM ".TABLE_MODULES."
	...
	{
		global $objLogin;
		$margin = "margin:-97px 0px 0px -28px;";
		...
		if($this->modulesCount > 0){			
			...
			echo draw_hidden_field($this->uPrefix."mg_action", "view")."\n";
			echo draw_hidden_field("mg_rid", "")."\n";
			echo draw_hidden_field("mg_sorting_fields", "id")."\n";
			echo draw_hidden_field("mg_sorting_types", "")."\n";
			echo draw_hidden_field("mg_page", "")."\n";
			echo draw_hidden_field("mg_operation", "")."\n";
			echo draw_hidden_field("mg_operation_type", "")."\n";
			echo draw_hidden_field("mg_operation_field", "")."\n";
			echo draw_hidden_field("mg_search_status", "")."\n";
			echo draw_hidden_field("mg_language_id", "")."\n";
				...
				if($this->modules[0][$i]['is_system'] == "1"){					
					if($objLogin->IsLoggedInAs($this->modules[0][$i]['settings_access_by'])){
						$modules_output .= "<div style='width:120px;float:".Application::Get("defined_left").";text-align:center;margin:5px;'>
								<b>".ucfirst(str_replace("_", " ", $this->modules[0][$i]['name']))."</b><br>
								<img src='images/modules_icons/".$this->modules[0][$i]['icon_file']."' alt='' style='margin:2px;border:1px solid #dedede'><br>
								".(($this->modules[0][$i]['is_installed'] == 1) ? "<img src='images/success_sign.gif' style='position:absolute;".$margin."' alt=\"\">" : "<img src='images/error_sign.gif' style='position:absolute;".$margin."' alt=\"\">")."
								<a href='javascript:void(0);' onclick='javascript:__mgDoPostBack(\"".$this->tableName."\", \"edit\", \"".$this->modules[0][$i]['id']."\");')\">[ "._EDIT_WORD." ]</a>
							</div>";
					}					
				}
			}
	...
	public function InstallModule()
	{
		...
		if($result[1] > 0){
			$first_module = ucfirst(str_replace("_", " ", $mid));
	...
	public function UninstallModule()
	{
		...
		{
			$sql = "SELECT name, module_tables, dependent_modules FROM ".TABLE_MODULES." WHERE name = '".$mid."'";
			$result = database_query($sql, DATA_AND_ROWS, FIRST_ROW_ONLY);
			if($result[1] > 0){
				// clear module tables
