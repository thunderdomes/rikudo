 ...
 *	PagesGrid
 *  -------------- 
 *  Description : encapsulates pages grid properties
 *	Usage       : MicroCMS, HotelSite, ShoppingCart 
 *  Updated	    : 20.04.2011
 *	Written by  : ApPHP
 *	Version     : 1.0.1
 *
...
class PagesGrid extends MicroGrid {
	...
	function __construct($type = "")
	{
		...
		$this->WHERE_CLAUSE = "WHERE
									...
									".$this->tableName.".language_id = '".$this->languageId."'"; // WHERE .... / "WHERE language_id = '".$this->languageId."'";				
		...
		foreach($total_menus[0] as $key => $val){
			$arr_menus[$val['id']] = $val['menu_name'];
		}		
		...
		$this->VIEW_MODE_SQL = "SELECT ".$this->tableName.".".$this->primaryKey.",
									...
									".$this->tableName.".page_title,
									".$this->tableName.".page_text,
									".$this->tableName.".menu_id,
									".$this->tableName.".menu_link,
									...
									CASE
										WHEN ".$this->tableName.".comments_allowed = 1 THEN
											...
											'</a>')
										ELSE
											'<font color=#b1b1b1>"._NOT_ALLOWED."</font>'
									END as comments_count,
									CONCAT('<a href=\"index.php?page=pages".(($type == "system") ? "&type=".$type: "")."&pid=', ".$this->tableName.".".$this->primaryKey.", '&mg_language_id=".$this->languageId."\">"._VIEW_WORD."</a> | <a href=\"index.php?admin=pages_edit".(($type != "") ? "&type=".$type: "")."&pid=', ".$this->tableName.".".$this->primaryKey.", '\">"._EDIT_WORD."</a> ".(($type == "system") ? "" : " | <a href=\"javascript:confirmRemoving(\'', ".$this->tableName.".".$this->primaryKey.", '\')\">"._REMOVE."</a>")."') as action_links
								FROM ".$this->tableName."
		...
		if($type == "system"){
			$this->arrViewModeFields["page_title"] = array("title"=>_PAGE_HEADER, "type"=>"label", "align"=>"left", "width"=>"", "maxlength"=>"50");
		}else{
			$this->arrViewModeFields["menu_link"] = array("title"=>_MENU_LINK, "type"=>"label", "align"=>"left", "width"=>"", "maxlength"=>"30");			
		}
