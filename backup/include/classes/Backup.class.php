...
class Backup {
	...
	public function ShowPreviousBackups($link_type = "delete", $draw = true)
	{
		...
		foreach($files as $key => $val){
			...
			if($link_type == "delete"){
				$output .= "<td>&nbsp;</td><td><a href='index.php?admin=mod_backup_installation&st=delete&fname=".$fname."' onclick='return confirm(\""._BACKUP_DELETE_ALERT."\")'>[ "._DELETE_WORD." ]</a></td><td>&nbsp;</td></tr>";				
			}else if($link_type == "restore"){
				$output .= "<td>&nbsp;</td><td><a href='index.php?admin=mod_backup_restore&st=restore&fname=".$fname."' onclick='return confirm(\""._BACKUP_RESTORE_ALERT."?\")'>[ "._RESTORE." ]</a></td><td>&nbsp;</td></tr>";				
			}else{
				//$output .= "";				
			}
		}
