...
class Comments extends MicroGrid {
	...
	{		
		...
		$this->VIEW_MODE_SQL = "SELECT ".$this->tableName.".".$this->primaryKey.",
									...
									IF(".$this->tableName.".date_created = '0000-00-00 00:00:00', '', ".$this->tableName.".date_created) as date_created,
		...
		$this->arrViewModeFields = array(
			"date_created"  => array("title"=>_DATE_CREATED, "type"=>"label", "align"=>"left", "width"=>"160px", "sortable"=>true, "nowrap"=>"", "visible"=>"", "height"=>"", "maxlength"=>"", "format"=>"date", "format_parameter"=>"M d, Y, g:i A"),
			...
			"user_email"      => array("title"=>_EMAIL_ADDRESS, "type"=>"label", "align"=>"center", "width"=>"130px", "sortable"=>true, "nowrap"=>"", "visible"=>"", "height"=>"", "maxlength"=>"", "format"=>""),
			"user_name"       => array("title"=>_USERNAME, "type"=>"label", "align"=>"center", "width"=>"", "sortable"=>true, "nowrap"=>"", "visible"=>"", "height"=>"", "maxlength"=>"", "format"=>""),
			// "parameter1"  => array("title"=>"", "type"=>"label", "align"=>"left", "width"=>"", "sortable"=>true, "nowrap"=>"", "visible"=>"", "height"=>"", "maxlength"=>"", "format"=>""),
			// "parameter2"  => array("title"=>"", "type"=>"image", "align"=>"center", "width"=>"", "sortable"=>true, "nowrap"=>"", "visible"=>"", "image_width"=>"50px", "image_height"=>"30px", "target"=>"uploaded/", "no_image"=>""),
		);
		...
		$this->EDIT_MODE_SQL = "SELECT
								...
								IF(".$this->tableName.".is_published = 1, '<span class=yes>"._YES."</span>', '<span class=no>"._NO."</span>') as m_is_published,
								IF(".$this->tableName.".date_created = '0000-00-00 00:00:00', '', ".$this->tableName.".date_created) as date_created,
								IF(".$this->tableName.".date_published = '0000-00-00 00:00:00', '', ".$this->tableName.".date_published) as m_date_published,
								".TABLE_PAGES.".page_title,
								".TABLE_LANGUAGES.".lang_name
							FROM ".$this->tableName."
								LEFT OUTER JOIN ".TABLE_PAGES." ON ".$this->tableName.".article_id = ".TABLE_PAGES.".id
		...
		$this->arrEditModeFields = array(
			...
			"date_created"  => array("title"=>_DATE_CREATED, "type"=>"label", "format"=>"date", "format_parameter"=>"M d, Y, g:i A"),
			"date_published" => array("title"=>"", "type"=>"hidden", "required"=>false, "default"=>date("Y-m-d H:i:s")),
			"m_date_published" => array("title"=>_DATE_PUBLISHED, "type"=>"label", "format"=>"date", "format_parameter"=>"M d, Y, g:i A"),
		...
		$this->arrDetailsModeFields = array(
			...
			"m_is_published" => array("title"=>_APPROVED, "type"=>"label"),
			"date_created"  => array("title"=>_DATE_CREATED, "type"=>"label", "format"=>"date", "format_parameter"=>"M d, Y, g:i A"),
			"m_date_published" => array("title"=>_DATE_PUBLISHED, "type"=>"label", "format"=>"date", "format_parameter"=>"M d, Y, g:i A"),
	...
	/**
	 *	Draw article comments
	 *		@param $article_id
	 *		@param $objCommentsSettings
	 */
	public function DrawArticleComments($article_id = "", $objCommentsSettings)
	{
		if(!$article_id) return "";
		
		global $objLogin;
		
		$delete_pending_time  = $objCommentsSettings->GetSettings("delete_pending_time");				
		$user_type  		= $objCommentsSettings->GetSettings("user_type");
		$comment_length		= $objCommentsSettings->GetSettings("comment_length");
		$image_verification = $objCommentsSettings->GetSettings("image_verification_allow");
		$comments_on_page   = $objCommentsSettings->GetSettings("page_size");
		$is_published       = ($objCommentsSettings->GetSettings("pre_moderation_allow") == "yes") ? "0" : "1";
		...
		if($task == "publish_comment"){
			...
			}else if(!check_email_address($user_email) && !@$objLogin->IsLoggedInAsClient()){
		...
		if($result[1] > 0){
			for($i=0; $i<$result[1]; $i++){
				...
				$output .= "<i>"._PUBLISHED.": ".$result[0][$i]["date_published"]."</i></div>";
				$output .= "</div>";
			}
			// draw pagination links
			if($total_pages > 1){
				$output .= "<div class='pagging'>";
				for($page_ind = 1; $page_ind <= $total_pages; $page_ind++){
					$output .= "<a class='pagging_link' href='index.php?page=".$page."&pid=".$pid."&p=".$page_ind."'>".(($page_ind == $current_page) ? "<b>[".$page_ind."]</b>" : $page_ind)."</a> ";
				}
				$output .= "</div>"; 
			}
		}else{
		...
		$output .= (($msg != "") ? $msg."<br />" : "");
		if($user_type == "registered" && !@$objLogin->IsLoggedInAsClient()){
			$output .= draw_message(_POST_COM_REGISTERED_ALERT, false);
		}else{
			$output .= $this->DrawCommentsForm($article_id, $image_verification, $focus_field, $task_completed);	
		}		
		$output .= "</div>\n";
	...
	/**
	 *	Draws comments submission form
	 *		@param $article_id
	 *		@param $image_verification
	 *		@param $focus_field
	 *		@param $task_completed
	 */
	private function DrawCommentsForm($article_id = "", $image_verification = "no", $focus_field = "", $task_completed = false)
	{
		if(!$article_id) return "";
		...
		<form class='comments-form' method='post' name='frmComments' id='frmComments'>
			...
			<tr>
				<td>";
				if($user_id == ""){
					$output .= _YOUR_NAME.": <input type='text' name='comment_user_name' id='comment_user_name' style='width:105px' value='".$user_name."' maxlength='50' autocomplete='off' />&nbsp;&nbsp;";
					$output .= "<nobr>"._YOUR_EMAIL.": <input type='text' name='comment_user_email' id='comment_user_email' style='width:105px' value='".$user_email."' maxlength='70' autocomplete='off' /></nobr><br /><br />";
				}else{
					$output .= "<input type='hidden' name='comment_user_name' id='comment_user_name' value='".$user_name."' /> <input type='hidden' name='comment_user_email' id='comment_user_email' value='".$user_email."' />";			
				}

				$output .= ""._COMMENT_TEXT.":<br />
				<textarea id='comment_text' name='comment_text' style='width:96%' rows='5'>".stripcslashes($comment_text)."</textarea><br />
				...
				<td valign='top' width='180px' align='center'>";
					if($image_verification == "yes"){						
						...
						<tr>
							<td colspan=2>
								<input type='text' name='captcha_code' id='captcha_code' style='width:175px' value='' autocomplete='off' />
							</td>
						</tr>
						</table>";						
					}
			...
			<input type='submit' ".($objLogin->IsLoggedInAsAdmin() ? "disabled" : "")." class='button' name='btnSubmitPC' id='btnSubmitPC' value='"._PUBLISH_YOUR_COMMENT."'>		
		</form>
		</div>";
	 ...
	 */
	static public function AwaitingModerationCount()
	{
