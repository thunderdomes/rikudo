...
class Menu {
	 ...
	 *		@param $lang_id
	 */
	static public function GetAll($order = " menu_order ASC", $join_table = "", $menu_placement = "", $lang_id = "")
	{
		$where_clause = "";
		if($menu_placement != ""){
	 ...
	 *		@param $language_id
	 */
	static public function DrawMenuSelectBox($menu_id = "", $language_id = "")
	{	
	 ...
	 *		@param $lang_id
	 */
	static private function GetAllFooter($where_clause = "", $lang_id = "")
	{
		global $objLogin;
	 ...
	 *		@param $content_type
	 */
	static public function DrawContentTypeBox($content_type = "")
	{
	 ...
	 *		@param $menu_placement
	 */
	static public function DrawMenuPlacementBox($menu_placement = "")
	{
		global $objSettings;
	 ...
	 *		@param $access_level
	 */
	static public function DrawMenuAccessSelectBox($access_level = "public")
	{
	 ...
	 *		@param $menu_position
	 */
	static public function DrawMenu($menu_position = "left")
	{
		...
		for($menu_ind = 0; $menu_ind < $menus[1]; $menu_ind++){				
			// Start draw new menu
			draw_block_top($menus[0][$menu_ind]['menu_name']);

			$menu_links = self::GetMenuLinks($menus[0][$menu_ind]['id'], $lang, $menu_position);
			if($menu_links[1] > 0) echo "<ul class='".Application::Get("lang_dir")."'>";
			for($menu_link_ind = 0; $menu_link_ind < $menu_links[1]; $menu_link_ind++) {
				if($menu_links[0][$menu_link_ind]['content_type'] == "link"){
					echo "<li>".prepare_permanent_link($menu_links[0][$menu_link_ind]['link_url'], $menu_links[0][$menu_link_ind]['menu_link'], $menu_links[0][$menu_link_ind]['link_target'], "main_menu_link")."</li>";
				}else{					
					// draw current menu link
					$class = (Application::Get("page_id") == $menu_links[0][$menu_link_ind]['id']) ? " active" : "";
					echo "<li>".prepare_link("pages", "pid", $menu_links[0][$menu_link_ind]['id'], $menu_links[0][$menu_link_ind]['page_key'], $menu_links[0][$menu_link_ind]['menu_link'], "main_menu_link".$class)."</li>";
				}
			}
		...
		// exit if logged and no preview allowed
		...
		if($menu_position == "left"){
			if(Modules::IsModuleInstalled("clients")){
				...
				if($objClientsSettings->GetSettings("show_login_block") == "yes"){
					$objClients = new Clients();
					$objClients->DrawLoginFormBlock();						
				}
			}
			...
			if(Modules::IsModuleInstalled("booking")){
				...
				if($objCartSettings->GetSettings("is_active") == "yes"){
					...
					echo "<div class='payment_instruments'><img src='images/ppc_icons/logo_paypal.gif' border='0' title='PayPal' alt='PayPal' />
						  ...
						  <img src='images/ppc_icons/logo_ccAmex.gif' border='0' title='Amex' alt='Amex' /></div>";
					draw_block_bottom();									
				}
			}

			// Draw local time
			draw_block_top(_LOCAL_TIME);
				Hotel::DrawLocalTime();
			draw_block_bottom();												
	 ...
	 *	Draws top menu
	 */
	static public function DrawTopMenu()
	{		
		echo "<li><a href='index.php'>"._HOME."</a></li>";			

		if(Modules::IsModuleInstalled("clients")){			
			echo "<li><a href='index.php?client=my_account'>"._MY_ACCOUNT."</a></li>";
		}
		if(Modules::IsModuleInstalled("booking")){				
			$objCartSettings = new ModulesSettings("booking");
			if($objCartSettings->GetSettings("is_active") == "yes"){
	 ...
	 */
	static public function DrawFooterMenu()
	{
		$lang = Application::Get("lang");
		...
		for($ind = 0; $ind < $system_pages[1]; $ind++) {
			if(($system_pages[0][$ind]['is_published']) &&
			   ($system_pages[0][$ind]['system_page'] == "terms" ||
				$system_pages[0][$ind]['system_page'] == "about_us" ||
				$system_pages[0][$ind]['system_page'] == "contact_us")
				){
				if($system_pages[0][$ind]['content_type'] == "link"){
					$output .= "&nbsp;&nbsp;".get_divider()."&nbsp;&nbsp;".prepare_permanent_link($system_pages[0][$ind]['link_url'], $system_pages[0][$ind]['menu_link'], $system_pages[0][$ind]['link_target']);
				}else{					
					$output .= "&nbsp;&nbsp;".get_divider()."&nbsp;&nbsp;".prepare_link("pages", "system_page", $system_pages[0][$ind]['system_page'], "index", $system_pages[0][$ind]['page_title'], "");
				}
			}
		}
		$output .= "<br />";
		...
		for($ind = 0; $ind < $system_pages[1]; $ind++) {
			if(($system_pages[0][$ind]['is_published']) &&
			   ($system_pages[0][$ind]['system_page'] != "terms" &&
				...
				if($ind != 0) $output .= "&nbsp;&nbsp;".get_divider()."&nbsp;&nbsp;"; 
				if($system_pages[0][$ind]['content_type'] == "link"){
					$output .= prepare_permanent_link($system_pages[0][$ind]['link_url'], $system_pages[0][$ind]['menu_link'], $system_pages[0][$ind]['link_target']);
				}else{					
					$output .= prepare_link("pages", "system_page", $system_pages[0][$ind]['system_page'], "index", $system_pages[0][$ind]['page_title'], "");
				}				
			}
		}
		if(Modules::IsModuleInstalled("booking")) $output .= "&nbsp;&nbsp;".get_divider()."&nbsp;&nbsp;<a href='index.php?page=booking'>"._BOOKING."</a>";
		$output .= "<br />";
			
		$menus = self::GetAllFooter("", $lang);
		if($menus[1] > 0) echo "<br />";
		for($menu_ind = 0; $menu_ind < $menus[1]; $menu_ind++) {
			if($menu_ind > 0) $output .= "&nbsp;&nbsp;".get_divider()."&nbsp;&nbsp;";
	 ...
	 *		@param $lang_id
	 */
	static public function GetTopMenus($lang_id = "")
	{
		global $objLogin;
	 ...
	 *		@param $lang_id
	 */
	static public function GetMenuPages($menu_id = "0", $lang_id = "")
	{
		global $objLogin;
	 ...
	 *		@param $position
	 */
	static private function GetMenuLinks($menu_id, $lang_id = "", $position = "left")
	{
		global $objLogin;
	 ...
	 *		@param $position
	 */
	static public function GetMenus($lang_id = "", $position = "left")
	{
		global $objLogin;
		...
		$sql = "SELECT
				...
				FROM ".TABLE_MENUS."
				    INNER JOIN ".TABLE_LANGUAGES." ON ".TABLE_MENUS.".language_id = ".TABLE_LANGUAGES.".abbreviation
				WHERE
					".TABLE_MENUS.".language_id = '".$lang_id."' AND
	 ...
	 */
	static public function GetAllSystemPages()
	{
		$sql = "SELECT ".TABLE_PAGES.".* 
				FROM ".TABLE_PAGES."
				WHERE
	 ...
	 */
	static public function DrawHeaderMenu()
	{
		...
		$output = "<ul class='nav nav_bg_".Application::Get("lang_dir")."'>";
		...
		for($ind = 0; $ind < $system_pages[1]; $ind++) {
			if(($system_pages[0][$ind]['is_published']) &&
				...
				if($system_pages[0][$ind]['content_type'] == "link"){
					...
					$output .= "<li>".prepare_link("pages", "system_page", $system_pages[0][$ind]['system_page'], "index", $system_pages[0][$ind]['page_title'], (($system_page == $system_pages[0][$ind]['system_page']) ? "class='current'" : ""))."</li>";
				}
			}
		}
		if(Modules::IsModuleInstalled("booking")) $output .= "<li><a href='index.php?page=booking' ".(($system_page == "booking") ? "class='current'" : "").">"._BOOKING."</a></li>";
	 ...
	 *		@param $lang_id
	 */
	static public function GetAllTop($where_clause = "", $lang_id = "")
	{
		global $objLogin;
