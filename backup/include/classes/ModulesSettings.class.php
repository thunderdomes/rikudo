 ...
 *	1.0.3 :
 *		- added displaying update message on error
 *		- Fixed bug in declaration of DrawEditMode
 *		- added InactiveDependentModules() method
 *		- added CanActivateModule() method
 *		- changed settings_value sanitize function	
 *	1.0.2 :
 *		- added colored value for enum in Details Mode
 *		- addede colored yes, no values for enum type
 *		- added "text" field type - WHEN key_display_type = 'text' THEN '' in viewmode
 *		- added required/non-required fields ALTER TABLE `mb_modules_settings` ADD `key_required` TINYINT( 1 ) UNSIGNED NOT NULL DEFAULT '1' AFTER `key_display_type` 
 *	
 **/
...
class ModulesSettings extends MicroGrid {
	...
	{		
		$this->params = array();		
		if(isset($_POST['settings_description'])) $this->params['settings_description'] = prepare_input($_POST['settings_description']);
		...
		$this->VIEW_MODE_SQL = "SELECT ".$this->primaryKey.",
									...
									CASE
										...
										WHEN key_display_type = 'yes/no' AND settings_value = 'yes' THEN CONCAT('<font color=green>', UCASE(MID(settings_value,1,1)), MID(settings_value,2), '</font>')
										WHEN key_display_type = 'yes/no' AND settings_value = 'no' THEN CONCAT('<font color=darkred>', UCASE(MID(settings_value,1,1)), MID(settings_value,2), '</font>')
		...
		$this->arrViewModeFields = array(
			...
			"settings_value"       => array("title"=>_VALUE, "type"=>"label", "width"=>"15%", "align"=>"center")
		);
		...
		$this->arrAddModeFields = array(
			"settings_description" 	=> array("title"=>_PARAMETER,  "type"=>"textbox", "required"=>true),		
	...
	{		
		...
		echo draw_hidden_field("mg_rid", $rid)."\n";
		...
		if($this->isFilteringAllowed){
			foreach($this->arrFilteringFields as $key => $val){
				if($val['type'] == "text"){
					...
					echo draw_hidden_field("filter_by_".$key, $filter_field_value)."\n";
				}
			}
		}
		...
		foreach($this->arrEditModeFields as $key => $val){
			if(preg_match("/separator/i", $key) && is_array($val)){
				...
				echo "<legend>".$val['separator_info']['legend']."</legend>";
				echo "\n<table width='100%' border='0' cellspacing='0' cellpadding='2' class='mgrid_table'>\n";		
				foreach($val as $v_key => $v_val){
					if($v_key != "separator_info"){						
						echo "<tr>\n";
						echo "  <td width='27%'>".$v_val['title'].(($is_required) ? " <font color='#c13a3a'>*</font>" : "").":</td>\n";
						if(isset($this->params[$v_key]) && $this->params[$v_key] !== "" && ($v_val['type'] != "checkbox")){
							echo "  <td style='padding-left:6px;'>".$this->DrawFieldByType("edit", $v_key, $v_val, $this->params, false)."</td>\n";
						}else{
							echo "  <td style='padding-left:6px;'>".$this->DrawFieldByType("edit", $v_key, $v_val ,$result[0][0], false)."</td>\n";
						}
						echo "</tr>\n";
					}
				}
				...
				if($val['type'] != "hidden"){
					echo "<tr>\n";
					echo "  <td width='20%'>".$val['title'].(($is_required && $key == "settings_value") ? " <font color='#c13a3a'>*</font>" : "").":</td>\n";
						...
						if($row = database_query($sql, DATA_ONLY, FIRST_ROW_ONLY)){								
							if($row['key_display_type'] == "enum"){
								...
								foreach($key_display_source as $kds_key){
									$enum_source[$kds_key] = ucfirst($kds_key);
								}
								$val['type'] = "enum";
					...
					if(isset($this->params[$key]) && $this->params[$key] !== "" && ($val['type'] != "checkbox")){
						echo "  <td>".$this->DrawFieldByType("edit", $key, $val, $this->params, false)."</td>\n";													
					}else{
						echo "  <td>".$this->DrawFieldByType("edit", $key, $val, $result[0][0], false)."</td>\n";						
					}
					echo "</tr>\n";				
				}
			}
	...
	public function UpdateRecord($rid = "0")
	{
		...
		foreach($this->arrEditModeFields as $key => $val){
			...
			if($key == "settings_key"){
				...
				$sql = "SELECT key_display_type, key_is_required, settings_value 
						FROM ".$this->tableName."
				...
				if($row = database_query($sql, DATA_ONLY, FIRST_ROW_ONLY)){
					...
					if($row['key_display_type'] == "email" && !check_email_address($this->params['settings_value'])){
						$this->error = str_replace("_FIELD_", "PayPal Email", _FIELD_MUST_BE_EMAIL);
						return false;						
						...
						return false;
					}else if($row['key_display_type'] == "positive integer" && (!$this->IsInteger($this->params['settings_value']) || ($this->params['settings_value'] <= 0))){
						$this->error = str_replace("_FIELD_", $val['title'], _FIELD_MUST_BE_POSITIVE_INT);
						return false;						
					}else if($row['key_display_type'] == "unsigned float" && (!$this->IsFloat($this->params['settings_value']) || ($this->params['settings_value'] < 0))){
						$this->error = str_replace("_FIELD_", $val['title'], _FIELD_MUST_BE_UNSIGNED_FLOAT);
						return false;						
					}else if($row['key_display_type'] == "yes/no" && (strtolower($this->params['settings_value']) != "yes" && strtolower($this->params['settings_value']) != "no")){
						$this->error = str_replace("_FIELD_", $val['title'], _FIELD_MUST_BE_BOOLEAN);
						return false;						
					}
				}					
			}			
	 ...
	 *		@param $param_name
	 */
	public function GetSettings($param_name = "")
	{
		return isset($this->arrSettings[$param_name]['value']) ? $this->arrSettings[$param_name]['value'] : "";
	}	
	...
	public function CanActivateModule()
	{
		...
		if($result[1] > 0){
			if($result[0]['settings_key'] == "is_active" && $result[0]['settings_value'] == "no"){				
				$first_module = ucfirst(str_replace("_", " ", $this->moduleName));
				$second_module = ucfirst(str_replace("_", " ", $result[0]['name']));
