...
if(@$objLogin->IsLoggedInAs('owner','mainadmin')){
	...
	if($tabid == "1_1") {
		...
		$first_tab_content = '
			...
			<input type="hidden" name="submition_type" value="general" />
			<input type="hidden" name="tabid" value="'.$tabid.'" />
			<input type="hidden" name="rss_feed" value="1" />
			'.draw_token_field(true).'
			
			<table border="0" cellspacing="5" cellpadding="3" class="tabs_table">
			<tr valign="top">
				...
				<td colspan="2">
					<select name="is_offline">
						<option '.(($params["is_offline"] == "0") ? "selected" : "").' value="0">'._NO.'</option>
						<option '.(($params["is_offline"] == "1") ? "selected" : "").' value="1">'._YES.'</option>
					</select>		
				</td>
			</tr>
			<tr valign="top">
			...
			<tr valign="top">
				...
				<td colspan="2">
					<select name="seo_urls">
						<option '.(($params["seo_urls"] == "1") ? "selected" : "").' value="1">'._YES.'</option>
						<option '.(($params["seo_urls"] == "0") ? "selected" : "").' value="0">'._NO.'</option>
					</select>
				</td>
			</tr>
			<tr valign="top">
				...
				<td colspan="2">
					<select name="wysiwyg_type">
						<option '.(($params["wysiwyg_type"] == "none") ? "selected" : "").' value="none">None</option>
						<option '.(($params["wysiwyg_type"] == "openwysiwyg") ? "selected" : "").' value="openwysiwyg">openWYSIWYG</option>
						<option '.(($params["wysiwyg_type"] == "tinymce") ? "selected" : "").' value="tinymce">TinyMCE</option>
					</select>
				</td>
			</tr>
			<tr valign="top">
				<td nowrap>'._EMAIL_ADDRESS.'<font color="#c13a3a">*</font>: <img class="help" src="images/question_mark.png" alt="" title="'._ADMIN_EMAIL_ALERT.'" /> </td>
				<td align="left">
					<input type="text" name="admin_email" size="25" maxlength="70" value="'.$params["admin_email"].'" />
				</td>
				<td align="left" width="60%">
					'._MAILER.': <img class="help" src="images/question_mark.png" title="'._ADMIN_MAILER_ALERT.'">&nbsp;
					<select name="mailer">
						<option '.(($params["mailer"] == "php_mail_standard") ? "selected" : "").' value="php_mail_standard">PHP Mail Function ('._STANDARD.')</option>
						<option '.(($params["mailer"] == "php_mail_simple") ? "selected" : "").' value="php_mail_simple">PHP Mail Function ('._SIMPLE.')</option>
					</select>										
				</td>
			</tr>
			<tr valign="top">
				...
				<td colspan="2">
					<select name="date_format">
						<option '.(($params["date_format"] == "dd/mm/yyyy") ? "selected" : "").' value="dd/mm/yyyy">dd/mm/yyyy</option>
						<option '.(($params["date_format"] == "mm/dd/yyyy") ? "selected" : "").' value="mm/dd/yyyy">mm/dd/yyyy (american)</option>
					</select>
				</td>
			</tr>
			<tr valign="top">
				...
				<td colspan="2">
					<select name="rss_feed_type" id="rss_feed_type" '.((!$params["rss_feed"]) ? "disabled='disabled'" : "").'>
						<option '.(($params["rss_feed_type"] == "rss1") ? "selected" : "").' value="rss1">RSS 1.0</option>
						<option '.(($params["rss_feed_type"] == "rss2") ? "selected" : "").' value="rss2">RSS 2.0</option>
						<option '.(($params["rss_feed_type"] == "atom") ? "selected" : "").' value="atom">Atom</option>
					</select>					
				</td>
			</tr>
			<tr valign="top">
				...
				<td colspan="2">
					<select name="caching_allowed" onchange="appToggleElementView(this.value, \'0\', \'cache_lifetime_row\')">
						<option '.(($params["caching_allowed"] == "1") ? "selected" : "").' value="1">'._YES.'</option>
						<option '.(($params["caching_allowed"] == "0") ? "selected" : "").' value="0">'._NO.'</option>
					</select>
					&nbsp;
			...
			<tr valign="top" id="cache_lifetime_row" style="'.(($params["caching_allowed"] == "1") ? "" : "display:none").';">
				...
				<td colspan="2">'.draw_numbers_select_field("cache_lifetime", $params["cache_lifetime"], 1, 60, 1, "", true).' '._MINUTES.'.</td>
			</tr>
		...
		$second_tab_content = '
			<form name="frmSettings" id="frmSettings" action="index.php?admin=settings" method="post">
				<input type="hidden" name="tabid" value="'.$tabid.'" />
				'.draw_token_field(true).'
				
				<table width="100%" border="0" cellspacing="5" cellpadding="3" class="main_text">
				<tr valign="top">
					<td width="150px">'._LANGUAGE.':</td>
					<td>';				
					$all_languages = Languages::GetAllActive();		
					$second_tab_content .= get_languages_box("sel_language_id", $all_languages[0], "abbreviation", "lang_name", $language_id, "", "onchange='javascript:appFormSubmit(\"frmSettings\");'");		
					$second_tab_content .= '</td>
				</tr>
				</table>
			</form>
			...
			<legend>'._HEADERS_AND_FOOTERS.'</legend>
				<form action="index.php?admin=settings" method="post">				
				<input type="hidden" name="submition_type" value="visual_settings" />
				<input type="hidden" name="tabid" value="'.$tabid.'" />
				<input type="hidden" name="sel_language_id" value="'.$language_id.'" />
				'.draw_token_field(true).'
				
				<table width="100%" border="0" cellspacing="5" cellpadding="3" class="tabs_table">
				<tr valign="top">
					<td width="150px">'._HDR_HEADER_TEXT.' <font color="#c13a3a">*</font>:</td>
					<td><textarea class="form_text" name="header_text" id="header_text" cols="54" rows="3">'.decode_text($params_tab2["header_text"]).'</textarea></td>
				</tr>
				<tr valign="top">
					<td width="150px">'._HDR_SLOGAN_TEXT.':</td>
					<td><textarea class="form_text" name="slogan_text" id="slogan_text" cols="54" rows="3">'.decode_text($params_tab2["slogan_text"]).'</textarea></td>
				</tr>
				<tr valign="top">
					<td width="150px">'._HDR_FOOTER_TEXT.' <font color="#c13a3a">*</font>:</td>
					<td><textarea class="form_text" name="footer_text" id="footer_text" cols="54" rows="3">'.decode_text($params_tab2["footer_text"]).'</textarea></td>
				</tr>
			...
			<legend>'._META_TAGS.'</legend>
				<form action="index.php?admin=settings" method="post">
				<input type="hidden" name="submition_type" value="meta_tags" />
				<input type="hidden" name="tabid" value="'.$tabid.'" />
				<input type="hidden" name="sel_language_id" value="'.$language_id.'" />
				'.draw_token_field(true).'
				
				<table border="0" cellspacing="4" cellpadding="3" class="tabs_table">
				<tr valign="top">
					<td width="150px">'._TAG.' &lt;TITLE&gt; <font color="#c13a3a">*</font>:</td>
					<td><textarea class="form_text" name="tag_title" id="tag_title" cols="54" rows="2">'.$params_tab3["tag_title"].'</textarea></td>
				</tr>
				<tr valign="top">
					<td width="150px">'._META_TAG.' &lt;KEYWORDS&gt;:</td>
					<td><textarea class="form_text" name="tag_keywords" id="tag_keywords" cols="54" rows="3">'.$params_tab3["tag_keywords"].'</textarea></td>
				</tr>
				<tr valign="top">
					<td width="150px">'._META_TAG.' &lt;DESCRIPTION&gt;:</td>
					<td><textarea class="form_text" name="tag_description" id="tag_description" cols="54" rows="3">'.$params_tab3["tag_description"].'</textarea></td>
				</tr>
				<tr>
		...
		$third_tab_content = '	
			<form action="index.php?admin=settings" method="post">
			<input type="hidden" name="submition_type" value="templates" />
			<input type="hidden" name="tabid" value="'.$tabid.'" />
			'.draw_token_field(true).'
			
			<table border="0" cellspacing="5" cellpadding="3" class="tabs_table">
			<tr valign="top">
				<td width="80px">'._HDR_TEMPLATE.' <font color="#c13a3a">*</font>:</td>
				<td width="100px">
					...
					<option value="">-- '._SELECT.' --</option>';					
						...
						foreach($arr_templates as $key){
							$third_tab_content .= "<option ".(($template == $key) ? "selected" : "")." value='".$key."'>".ucfirst($key)."</option>";
						}
						$third_tab_content .= '
					</select>
				</td>
			...
			<tr><td colspan="4" nowrap height="5px"></td></tr>
			<tr>
		...
		$fifth_tab_content = '
			<form action="index.php?admin=settings" method="post">
				<input type="hidden" name="submition_type" value="site_info" />
				<input type="hidden" name="tabid" value="'.$tabid.'" />
				'.draw_token_field(true).'
		...
		$sixth_tab_content = '
			<form action="index.php?admin=settings" method="post">
				<input type="hidden" name="submition_type" value="cron_settings" />
				<input type="hidden" name="tabid" value="'.$tabid.'" />
				'.draw_token_field(true).'
				
				<table width="100%" border="0" cellspacing="5" cellpadding="5" class="main_text">
				<tr valign="top">
...
&lt;/Files&gt;
				...
				<tr valign="top" id="cron_run_period_row" '.(($params_cron["cron_type"] != "non-batch") ? "style=\"display:none;\"" : "").'>
					<td>'._RUN_EVERY.':</td>
					<td>
						'.draw_numbers_select_field("cron_run_period_value", $params_cron["cron_run_period_value"], 1, 100, 1, "", true).'
				...
				<tr><td colspan="3" nowrap height="15px"></td></tr>
			</form>';			
	}
	...
	$tab1=$tabs->AddTab(_GENERAL_SETTINGS, $first_tab_content);
	$tab2=$tabs->AddTab(_VISUAL_SETTINGS, $second_tab_content);
	$tab3=$tabs->AddTab(_TEMPLATES_STYLES, $third_tab_content);
	$tab4=$tabs->AddTab(_SERVER_INFO, $fourth_tab_content);
	$tab5=$tabs->AddTab(_SITE_INFO, $fifth_tab_content);
	$tab6=$tabs->AddTab(_CRON_JOBS, $sixth_tab_content);
