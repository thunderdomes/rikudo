...
if(@$objLogin->IsLoggedInAsAdmin()){
	...
	if($act == "edit"){
		...
		$params['finish_publishing']    = (isset($_POST['finish_publishing']) && check_date($_POST['finish_publishing']) && $_POST['finish_publishing'] != _NEVER) ? prepare_input($_POST['finish_publishing']) : "0000-00-00";
		
		if($objPage->PageUpdate($params)){
			$msg = draw_success_message(_PAGE_SAVED, false);
			if(Application::Get("page_id") != "home"){
				$user_session->SetMessage('notice', $msg);				
				header("location: index.php?admin=pages".((!empty($type)) ? "&type=".$type: "")."&mg_language_id=".$params['language_id']);							
			}
		}else{			
