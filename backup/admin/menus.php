...
if(@$objLogin->IsLoggedInAs('owner','mainadmin')){
	...
	draw_title_bar(prepare_breadcrumbs(array(_MENU_MANAGEMENT=>"",_EDIT_MENUS=>"")));
...
?>    
	...
	<!--
		function confirmDelete(mid, mo){
			if(!confirm('<?php echo _MENU_DELETE_WARNING;?>')){
				...
				document.location.href = 'index.php?admin=menus&act=delete&mid='+mid+'&mo='+mo+'&language_id=<?php echo $language_id; ?>';
			}			
		}
	//-->
	</script>
	...
	<tr valign="middle">
		...
		<td align='right' width="80px">
			<?php echo get_languages_box("language_id", $total_languages[0], "abbreviation", "lang_name", $language_id, "", "onchange=\"document.location.href='index.php?admin=menus&language_id='+this.value\""); ?>
		</td>
	</tr>
	...
	if($all_menus[1] > 0){ ?>				
		...
		<tr>
			...
			<th width="9%"><?php echo _ACTIONS;?></th>
		</tr>
		...
		<?php
			for($i=0;$i<$all_menus[1];$i++){				
				echo "<tr ".highlight(0)." onmouseover='oldColor=this.style.backgroundColor;this.style.backgroundColor=\"#e1e1e1\";' onmouseout='this.style.backgroundColor=oldColor'>
					...
					<td align='center'>".$all_menus[0][$i]['menu_placement']."</td>
					<td align='center'>".$all_menus[0][$i]['menu_order']."</td>
					<td align='center'>
						<a href='index.php?admin=menus&act=move&mid=".$all_menus[0][$i]['id']."&mo=".$all_menus[0][$i]['menu_order']."&dir=up&language_id=".$language_id."'>"._UP."</a>/<a href='index.php?admin=menus&act=move&mid=".$all_menus[0][$i]['id']."&mo=".$all_menus[0][$i]['menu_order']."&dir=down&language_id=".$language_id."'>"._DOWN."</a>
					</td>
					<td align='center' nowrap>
						<a href='index.php?admin=menus_edit&mid=".$all_menus[0][$i]['id']."&language_id=".$language_id."'>"._EDIT_WORD."</a>&nbsp;".get_divider()."&nbsp;<a href='javascript:confirmDelete(\"".$all_menus[0][$i]['id']."\", \"".$all_menus[0][$i]['menu_order']."\");'>"._DELETE_WORD."</a>
					</td>
				</tr>";
			}
		echo "</table>";
	}else{
		echo draw_message(_MENU_NOT_FOUND);
	} 
	draw_content_end();	
