...
if(@$objLogin->IsLoggedInAsAdmin() && Modules::IsModuleInstalled("booking")){
	...
	// Start main content
	draw_title_bar(
		prepare_breadcrumbs(array(_BOOKING=>"",_ROOMS_AVAILABILITY=>"")),
		_LOCAL_TIME.": ".(($objSettings->GetParameter("date_format") == "mm/dd/yyyy") ? date("M d, Y H:i:s") : date("d M, Y H:i:s"))
	);
	...
	switch(Application::Get("lang")){
		case "de":
			$objCalendar->SetInterfaceLang("de");    
			break;
		case "es":
