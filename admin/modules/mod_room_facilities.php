...
if($objLogin->IsLoggedInAs('owner','mainadmin') && Modules::IsModuleInstalled('booking')){
	...
	// Start main content
	draw_title_bar(prepare_breadcrumbs(array(_HOTELS_MANAGEMENT=>'',_SETTINGS=>'',_ROOM_FACILITIES_MANAGEMENT=>'',ucfirst($action)=>'')));
	...
	if($mode == 'view'){		
		...
		$objRoomFacilities->DrawEditMode($rid, $operation, $operation_field);		
