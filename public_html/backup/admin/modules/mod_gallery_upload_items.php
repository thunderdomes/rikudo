...
if(@$objLogin->IsLoggedInAsAdmin() && Modules::IsModuleInstalled("gallery")){	
	...
	if(count($album_info) > 0){
		...
		// Start main content
		draw_title_bar(
			prepare_breadcrumbs(array(_MODULES=>"",_GALLERY_MANAGEMENT=>"",_ALBUM=>"",$album_info[0]['name']=>"",ucfirst($action)=>"")),
			"<a href='index.php?admin=mod_gallery_management'>"._BUTTON_BACK."</a>"
		);
