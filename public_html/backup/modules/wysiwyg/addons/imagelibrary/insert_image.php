...
if(@$objLogin->IsLoggedInAsAdmin()){
	...
	$wysiwyg = isset($_GET['wysiwyg']) ? prepare_input($_GET['wysiwyg']) : ""; 
	// set image dir
	$leadon = $imagebasedir;
	...
	$get_dir = isset($_REQUEST['dir']) ? prepare_input($_REQUEST['dir']) : "";
	if($get_dir){
		if(substr($get_dir, -1, 1)!='/') {
			$get_dir = $get_dir.'/';
		}
		$dirok = true;
		$dirnames = split('/', $get_dir);
...
?>
	...
	<tr>
		<td colspan="2" align="right" style="padding-top: 5px;">
			<input type="button" value="  Submit  " onclick="insertImage();" style="font-size: 12px;">
			<?php if ( $allowuploads ) { ?> 
